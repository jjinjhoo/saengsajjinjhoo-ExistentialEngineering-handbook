...
[Error Code 404] 목적지를 찾을 수 없음 (무기력): 방향성(선택) 데이터 누락. 당장 1주일 단위의 작은 실행(Run)을 돌려 더미 데이터라도 쌓을 것.

[Error Code 502] 외부 서버 응답 불량 (타인에게 상처받음): 타인의 비난은 내 서버 문제가 아니라 상대방 내부의 컴파일 에러임. 내 변수가 아닌 상수이므로 수신 거부(Drop) 처리할 것.
